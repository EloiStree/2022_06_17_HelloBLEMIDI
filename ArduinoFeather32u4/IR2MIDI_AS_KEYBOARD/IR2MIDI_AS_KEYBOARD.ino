...
    FACTORYRE§èSET_ENABLE       Perform a factory reset when running this sketch
   
...
void setup(void) {
  ...
  Serial.begin(115200);
  Serial.println(F("Adafruit Bluefruit HID Keyboard Example and midi"));
  Serial.println(F("---------------------------------------"));
  ...
  if (!ble.sendCommandCheckOK(F("AT+GAPDEVNAME=Eloi Bluetooth Keyboard"))) {
...
void loop(void) {
  ...
  if (IrReceiver.decode()) {
    ...
    if (IrReceiver.decodedIRData.protocol == UNKNOWN) {
      ...
      int midiVelocity =126- (IrReceiver.decodedIRData.address * 2);
      int channel = 1;
...

      {
        ...
        // send note on
        midi.send(0x90, midiNote, midiVelocity);
        //MIDI.sendNoteOn(midiNote, midiVelocity, channel);

        delay(50);

        // send note off
        midi.send(0x80, midiNote, midiVelocity);
        // MIDI.sendNoteOff(midiNote, midiVelocity, channel);
    

        delay(2);

        // SEND KEYBOARD STROKE FROM TEXT
        // ble.print("AT+BleKeyboard=");
        // ble.println(current_note);
